...
#endif
      LogPrint("pow","\342\226%c", '\201' + sparks);
#endif
    }
    LogPrint("pow","\n");
#endif
...
void barrier(pthread_barrier_t *barry) {
  ...
  if (rc != 0 && rc != PTHREAD_BARRIER_SERIAL_THREAD) {
    LogPrint("pow","Could not wait on barrier");
    pthread_exit(NULL);
  }
}
...
void *worker(void *vp) {
  ...
  for (u32 r = 1; r < WK; r++) {
    if (tp->id == 0)
      LogPrint("pow","Digit %d", r);
    barrier(&eq->barry);
    r&1 ? eq->digitodd(r, tp->id) : eq->digiteven(r, tp->id);
    barrier(&eq->barry);
    if (tp->id == 0) {
      LogPrint("pow"," x%d b%d h%d", eq->xfull, eq->bfull, eq->hfull);
      eq->xfull = eq->bfull = eq->hfull = 0;
      eq->showbsizes(r);
    }
    barrier(&eq->barry);
